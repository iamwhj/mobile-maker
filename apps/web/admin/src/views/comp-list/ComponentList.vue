<template>
  <div class="component-list">
    <el-button type="primary" class="add-btn" @click="add"> 添加组件 </el-button>
    <el-table :data="componentList">
      ...
      <el-table-column label="操作" width="200">
        <template #default="scope">
          <el-link type="primary" @click="edit(scope.row)" style="margin-right: 10px"> 编辑 </el-link>
          <el-link type="danger" @click="del(scope.row.id)" disabled> 删除 </el-link>
        </template>
      </el-table-column>
    </el-table>
  </div>
  <el-dialog v-model="addVisible" title="添加组件" width="460px">
    <el-form :model="addComponent" label-width="90px">
      <el-form-item label="组件分类">
        <el-select v-model="addComponent.category_id" placeholder="请选择组件分类">
  ...
  <el-dialog v-model="editVisible" title="编辑组件" width="460px">
    <el-form :model="editComponent" label-width="90px">
      <el-form-item label="组件分类">
        <el-select v-model="editComponent.category_id" placeholder="请选择组件分类">
          <el-option v-for="item in categoryList" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="组件代码">
...
<script setup>
import { ref } from 'vue'
import { removeFieldForMongodb } from '@mk/utils'
import { getCategoryList, getComponentList, updateComponent, deleteComponent, saveComponent } from '@mk/api'
...
// 组件上传地址
const action = import.meta.env.VITE_BUILD_URL + '/upload-component'
// 组件上传格式校验
