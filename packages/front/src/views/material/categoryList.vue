<template>
  ...
  <el-dialog v-model="editVisible" title="编辑分类" width="460px">
    <el-form :model="editCategory.data" label-width="90px">
      <el-form-item label="分类名称">
        <el-input v-model="editCategory.data.name" />
      </el-form-item>
      <el-form-item label="分类排序">
        <el-input v-model.number="editCategory.data.priority" />
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
...
const edit = (data) => {
  ...
  editCategory.value.id = data.id;
  editVisible.value = true;
};
const editCategoryFn = () => {
  updateCategory({
    id: editCategory.value.id,
    data: editCategory.value.data,
  }).then((result) => {
    const res = result.data;
    if (res.code === 0) {
      ElMessage({
